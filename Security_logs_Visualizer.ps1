<#
    ...
    - ADDED: "Clear Filters" button next to "Apply"
    - FIXED: XML Type Mismatch (XmlElement vs XmlDocument)
    - FIXED: Isolated SHA256 from Sysmon hash strings
    - UPDATED: Enabled Cell Selection for Copy/Paste (Ctrl+C)
    - PERFORMANCE: Optimized HTML Engine & Date Filtering
#>
...
function Get-CombinedEDREvents {
    ...
    $parsedData = foreach ($e in $events) {
        ...
        elseif ($data.Hashes -match '^[A-Fa-f0-9]{64}$') { $sha256 = $data.Hashes }
